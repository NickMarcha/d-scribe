...
pub fn export_srt(path: &Path, segments: &[SessionSegment], texts: &[String]) -> Result<(), String> {
    srt::write_srt(path, segments, texts)
}

/// Export transcript segments to VTT format.
pub fn export_vtt(path: &Path, segments: &[SessionSegment], texts: &[String]) -> Result<(), String> {
